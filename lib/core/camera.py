...
class RgbdCamera(nn.Module):
    def __init__(self, cali_path, floor_path, dtype=torch.float32):
        ...
        # floor data
        floor_trans = nn.Parameter(torch.tensor(floor_trans_cpu,
                                                dtype=self.dtype),
                                   requires_grad=False)
        self.register_parameter('floor_trans', floor_trans)
        floor_normal = nn.Parameter(torch.tensor(floor_normal_cpu,
                                                 dtype=self.dtype),
                                    requires_grad=False)
        self.register_parameter('floor_normal', floor_normal)

        # 3d transform
        depth2color = nn.Parameter(torch.tensor(depth2color_cpu,
                                                dtype=self.dtype),
                                   requires_grad=False)
        self.register_parameter('depth2color', depth2color)
        depth2floor = nn.Parameter(torch.tensor(depth2floor_cpu,
                                                dtype=self.dtype),
                                   requires_grad=False)
        self.register_parameter('depth2floor', depth2floor)
        floor2color_cpu = depth2color_cpu @ np.linalg.inv(depth2floor_cpu)
        floor2color = nn.Parameter(torch.tensor(floor2color_cpu,
                                                dtype=self.dtype),
                                   requires_grad=False)
    ...
    def tranform3d(self, points, type):
        if not torch.is_tensor(points):
            points = torch.tensor(points,
                                  dtype=self.dtype).to(self.depth2color.device)
    ...
    def depth2PointCloud(self, depth_map, fx, fy, cx, cy):
        z = depth_map
    ...
    def preprocessDepth(self, depth_map, mask_map):
        ...
        depth_vraw = self.depth2PointCloud(depth_map, self.dIntr_cpu[0],
                                           self.dIntr_cpu[1],
                                           self.dIntr_cpu[2],
                                           self.dIntr_cpu[3])
